...
for _, col in ipairs(all_colours) do
	...
	local function horn_texture_sel(horns,gotten)		
		...
		if horns then
			textures = base_text.."^"..horn_text.."^("..wool_shave_text..col_text..")"
		end
	...
	mobs:register_mob("mobs_animal:sheep_" .. col[1], {
		...
		on_replace = function(self, pos, oldnode, newnode)
			...
			if self.food >= 8 then
				...
				-- base_texture must be kept upto date for static_save so horns persist visually on server/game restart
		...
		on_breed = function(parent1, parent2)
					...
					local horns = math.random(100) <= horn_chance
					...
					-- need to remove horns if it's in parent texture string, lambs dont have horns
					local textures = string.gsub(parent1.base_texture[1], "%^mobs_sheep_horns.png","")
				...
				-- stop mobs_redo from spawning child
				return false
		end,
	...
	mobs:spawn({
		...
		on_spawn = function(self, pos)
			...
			local function random_sheep(pos,first)							
				...
				-- store returned entity data from mobs:add_mob, "nil" indicates the rules in mobs:add_mob 
				-- stopped mob from being spawned/created.
				if lamb then
					entity = mobs:add_mob(pos, {name = "mobs_animal:sheep_" .. types, child = lamb})
						if entity then
							-- minetest.debug(types.." lamb: "..minetest.pos_to_string(pos,0))
						end
				else 
					entity = mobs:add_mob(pos, {name = "mobs_animal:sheep_" .. types})					
				end 
				...
				if entity and not lamb then
					...
					if horns then
						local text = "mobs_sheep_base.png^mobs_sheep_horns.png^(mobs_sheep_wool.png^[multiply:"..all_colours[nat_colors[types]][3]..")"
						entity.object:set_properties({textures = {text}})
						entity.base_texture = {text}
						entity.attribute_horns = horns
						-- minetest.debug(types.." horned sheep: "..minetest.pos_to_string(pos,0))
					else 
						-- minetest.debug(types.." hornless sheep: "..minetest.pos_to_string(pos,0))
					end
				end
			end
